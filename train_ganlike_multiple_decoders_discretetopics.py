...
def train_topic_predictor(c_model, t_model, data, optimizer, criterion, args, epoch, steps):
  ...
  accuracy_fromprompts, confusion_matrix_ = 0.0, np.zeros((args.nprompts, args.nprompts), dtype=int)
  t = time.time()
  total_topic_loss = 0
  num_batches = len(data)
  ...
  for batch_num, batch in enumerate(data):
    ...
    prompt_logits = t_model(textrep)

    prompt = batch.prompt
    topic_loss = criterion(prompt_logits.view(-1, args.nprompts), prompt)

    loss = topic_loss
    total_topic_loss += float(topic_loss)

    accuracy_fromprompts, confusion_matrix_ = update_stats(accuracy_fromprompts, confusion_matrix_, prompt_logits, prompt)
    loss.backward()
    optimizer.step()
  ...
  print("[Accuracy From Topics]: {}/{} : {}%".format(
        accuracy_fromprompts, np.sum(confusion_matrix_), accuracy_fromprompts / np.sum(confusion_matrix_) * 100))
...
def train_classifier(c_model, t_models, data, optimizer, classify_criterion, topic_criterion, args, epoch, steps):
  ...
  for batch_num, batch in enumerate(data):
    ...
    fake_topics = torch.ones((batch.prompt.size()[0], args.nprompts)).cuda() #want the model to predict uniform topics
    fake_topics = fake_topics.div(fake_topics.sum(dim=-1, keepdim=True))

    # print (fake_topics)

    real_prompts = batch.prompt
    ...
    # input("hello")
    prompt_logits = t_models[d_id](sentrep)
    prompt_logprobs = nn.LogSoftmax(dim=-1)(prompt_logits)
    ...
    topic_loss = klcriterion(prompt_logprobs, fake_topics)

    # print (prompt_logprobs*fake_topics)

    # print (topic_loss)
    # print (topic_criterion(prompt_logits.view(-1, args.nprompts), real_prompts))
    # raw_input()
    ...
    accuracy, confusion_matrix = update_stats(accuracy, confusion_matrix, logits, y)
    accuracy_fromprompts, confusion_matrix_ = update_stats(accuracy_fromprompts, confusion_matrix_, prompt_logprobs, real_prompts)
    loss.backward()
    optimizer.step()
  ...
  print("[Training Accuracy]: {}/{} : {}%".format(accuracy, np.sum(confusion_matrix), accuracy / np.sum(confusion_matrix) * 100))
...
def main():
  ...
  iters, TEXT, LABEL, PROMPTS = dataset_map[args.data](args.batch_size, device=device, vectors=vectors, base_path=args.base_path)
  ...
  if args.model == "CNN":
    ...
    topic_decoder = [nn.Sequential(nn.Linear(len(args.kernel_sizes)*args.kernel_num, args.nprompts))]

  else:
    ntokens, nlabels, nprompts = len(TEXT.vocab), len(LABEL.vocab), len(PROMPTS.vocab)
    ...
    if args.bottleneck_dim == 0:
      ...
      topic_decoder = [nn.Sequential(nn.Dropout(args.topic_drop), nn.Linear(attention_dim, args.nprompts))]
    else:
      ...
      topic_decoder = [nn.Sequential(nn.Dropout(args.topic_drop), nn.Linear(args.bottleneck_dim, args.nprompts))]
    ...
    try:
      ...
      for epoch in range(1, args.epochs + 1):
        ...
        if args.model == "CNN":
          ...
          attention_dim = args.hidden if not args.bi else 2*args.hidden
          if args.bottleneck_dim == 0:
            topic_decoder.append(nn.Sequential(nn.Dropout(args.topic_drop), nn.Linear(attention_dim, args.nprompts)))
          else:
            topic_decoder.append(nn.Sequential(nn.Dropout(args.topic_drop), nn.Linear(args.bottleneck_dim, args.nprompts)))
