...
def make_parser():
  ...
  parser.add_argument('--reset_topic_decoder', action='store_true',help="reset the topic decoder after every step")
...
def train_topic_predictor(c_model, t_model, data, optimizer, criterion, args, steps):
  ...
  print("[accuracy_fromtopics]: {}/{} : {}%".format(
...
def train_classifier(c_model, t_model, data, optimizer, classify_criterion, topic_criterion, args, steps):
  ...
  for batch_num, batch in enumerate(data):
    ...
    topic_logprobs = t_model(sentrep)
  ...
  print()
  print("[Fake Topic Loss]: {:.5f}".format(total_topic_loss / steps))
  print("[Loss]: {:.5f}".format(total_loss / steps))
  print("[Accuracy]: {}/{} : {}%".format(
        accuracy, np.sum(confusion_matrix), accuracy / np.sum(confusion_matrix) * 100))
  ...
  with torch.no_grad():
    ...
    for batch_num, batch in enumerate(data):
      ...
      if args.write_attention and itos is not None:
        ...
        for sentence, length, attns, ll, mi in zip(x.permute(1,0).cpu().data.numpy(), lens.cpu().data.numpy(), energy, y.cpu().data.numpy(), max_ind.cpu().data.numpy()):
          s = ""
...
def main():
  ...
  for p in topic_decoder.parameters():
    if not p.requires_grad:
      print ("OMG", p)
      p.requires_grad = True
    p.data.uniform_(-args.param_init, args.param_init)
  ...
  topic_optim.set_parameters(topic_decoder.parameters())
  ...
  if args.load:
    ...
    try:
      ...
      for epoch in range(1, args.epochs + 1):
        #if this is true, it trains a new topic decoder after every step, so that it's not overfitting
        if args.reset_topic_decoder:
          for p in topic_decoder.parameters():
            p.data.uniform_(-args.param_init, args.param_init)

        for t_step in range(1, args.t_steps+1):
          train_topic_predictor(classifier_model, topic_decoder, train_iter, topic_optim, topic_criterion, args, args.t_steps)

        for c_step in range(1, args.c_steps+1):
          train_classifier(classifier_model, topic_decoder, train_iter, classify_optim, classify_criterion, topic_criterion, args, args.c_steps)
          loss = evaluate(classifier_model, val_iter, classify_criterion, args)
          oodLoss = evaluate(classifier_model, outdomain_test_iter[0], classify_criterion, args, datatype="oodtest")
